%% ========================================================================
%  Analisis del barrido de frecuencia de SALIDA (tb_SVM_FoutVar)
%
%  Testbench: entrada trifasica FIJA en 50 Hz, frecuencia deseada de salida
%  (rampa i_al_o) en escalones de 40, 50 y 60 Hz, 0,2 s cada uno.
%
%  Lo que se busca contestar: la salida SIGUE la frecuencia comandada, y
%  cuanto se degrada el espectro al alejarse de la frecuencia de entrada.
%
%  Los escalones se detectan con la columna Freq_Hz del CSV, no con tiempos
%  hardcodeados.
%
...
ARCH_TENSIONES   = 'Clk10M_50i_FoVar.csv';
ARCH_DIRECCIONES = 'w_direcciones_FoVar.csv';

Fs = 1e6;               % muestreo del testbench: 1 de cada 10 flancos de clk
Ts = 1/Fs;

F_IN = 50;              % fuente de entrada, FIJA en este testbench [Hz]
                        % las de salida salen de la columna Freq_Hz
...
if ~ismember('Freq_Hz', datos.Properties.VariableNames)
    error(['El CSV no tiene columna Freq_Hz. Este script espera la salida de ' ...
           'tb_SVM_FoutVar.vhd, no la de tb_SVM_Wrapper.vhd.']);
end
f_cmd = datos.Freq_Hz;
...
for k = 1:n_esc
    idx = lim(k):lim(k+1)-1;
    fprintf('  %d) f_out = %3d Hz   t = [%.4f  %.4f] s   (%d muestras)\n', ...
            k, f_cmd(idx(1)), t(idx(1)), t(idx(end)), numel(idx));
end

%% ------------------------- Panorama temporal ----------------------------

figure('Name', 'Barrido de f_out: tensiones de salida', 'Color', 'w');
...
xlabel('Tiempo [s]'); ylabel('f_{out} [Hz]');
title(sprintf('Frecuencia comandada a la salida (entrada fija en %g Hz)', F_IN));
...
figure('Name', 'Barrido de f_out: corrientes en la carga RL', 'Color', 'w');
plot(t, i_u, 'r', t, i_v, 'g', t, i_w, 'b');
grid on; xlim([0 t(end)]);
marcar_escalones(t, lim);
legend('i_U','i_V','i_W');
xlabel('Tiempo [s]'); ylabel('Corriente [A]');
title(sprintf(['Corrientes con R = %.2f Ohm, L = %.1f mH  -  la amplitud baja ' ...
               'al subir f porque la carga es inductiva'], R_CARGA, L_CARGA*1e3));
...
figure('Name', 'Barrido de f_out: espectros por escalon', 'Color', 'w');
...
for k = 1:n_esc
    ...
    % --- Bandas de interes ---------------------------------------------
    % Nombre, frecuencia. El fundamental va primero: todo se reporta
    % relativo a el.
    bandas = { ...
    ...
    fprintf('\n--- Escalon %d/%d : f_out comandada = %g Hz (f_in = %g Hz) ---\n', ...
            k, n_esc, F_OUT, F_IN);
    ...
    title(sprintf('Escalon %d: f_{out} = %g Hz (medida %.2f Hz), residuo < %g Hz = %.2f %%', ...
                  k, F_OUT, f_medida(k), F_RESIDUO, residuo_pct(k)));
end

%% ------------------------- Seguimiento de la referencia -----------------
% El resultado central de este testbench.

figure('Name', 'Barrido de f_out: seguimiento', 'Color', 'w');

subplot(1,2,1);
plot(f_comandada, f_medida, 'bo-', 'LineWidth', 1.4, 'MarkerFaceColor', 'b');
hold on;
ideal = [min(f_comandada)-5, max(f_comandada)+5];
plot(ideal, ideal, 'k--');
hold off; grid on; axis square;
xlim(ideal); ylim(ideal);
legend('Medida', 'Ideal (y = x)', 'Location', 'northwest');
xlabel('f_{out} comandada [Hz]'); ylabel('f_{out} medida [Hz]');
title('Seguimiento de la frecuencia de salida');

subplot(1,2,2);
bar(f_comandada, f_medida - f_comandada, 0.5);
grid on;
xlabel('f_{out} comandada [Hz]'); ylabel('Error [Hz]');
title('Error de frecuencia por escalon');

fprintf('\n--- Resumen del barrido ---\n');
fprintf('  %10s %12s %12s %12s %12s\n', ...
        'f_cmd [Hz]', 'f_med [Hz]', 'error [Hz]', 'fundamental', 'residuo [%]');
for k = 1:n_esc
    fprintf('  %10g %12.3f %12.3f %12.6f %12.2f\n', ...
            f_comandada(k), f_medida(k), f_medida(k)-f_comandada(k), ...
...
if isfile(ARCH_DIRECCIONES)
    ...
    figure('Name', 'Barrido de f_out: llaves de la matriz', 'Color', 'w');
    paso = 1.5;
    for k = 1:n_esc
        ...
        ylim([-0.5, 9*paso]);
        xlabel('Tiempo [s]');
        title(sprintf('f_{out} = %g Hz  -  zoom de %g ms en el centro del escalon', ...
                      F_OUT, T_ZOOM*1e3));
    end
else
