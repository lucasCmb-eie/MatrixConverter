...
-- tb_SVM_FinVar
--
-- Variante de tb_SVM_Wrapper con la SALIDA FIJA en 50 Hz (rampa i_al_o de la
-- carga) y la frecuencia de la FUENTE TRIFASICA DE ENTRADA recorriendo
-- escalones de 40, 50 y 60 Hz, 0,2 s cada uno (0,6 s de simulacion en total).
--
-- Los tres acumuladores de fase de la fuente reciben el MISMO incremento, asi
-- que los desfases de +-120 grados se conservan a traves de los escalones, y
-- ninguno se reinicia: la fase es continua y el salto de frecuencia no mete
-- una discontinuidad artificial en la tension de entrada.
--
-- Salidas a CSV (decimadas x10 -> Fs = 1 MHz):
--   Clk10M_FiVar_50o.csv     Muestra,Tension_U,Tension_V,Tension_W,Freq_Hz
--   w_direcciones_FiVar.csv  Muestra,Direcciones
-- Freq_Hz es la frecuencia de la fuente de ENTRADA en ese instante.
--
...
entity tb_SVM_FinVar is
end tb_SVM_FinVar;

architecture Behavioral of tb_SVM_FinVar is
    ...
    signal test_al_o : std_logic_vector(10 downto 0); -- rampa de salida fija (50 Hz)
    ...
    -- ---------------------------------------------------------------------
    -- Escalones de frecuencia de ENTRADA
    -- ---------------------------------------------------------------------
    ...
    signal ftw_in      : unsigned(31 downto 0);
    signal freq_hz     : integer;

    -- Rampa de la referencia de salida deseada, FIJA en 50 Hz
    constant FREQ_TUNING_WORD_OUT : unsigned(31 downto 0) := to_unsigned(21475, 32); -- 50 Hz
...
begin
    ...
    -- ---------------------------------------------------------------------
    -- Secuenciador de escalones de frecuencia de entrada
    -- ---------------------------------------------------------------------
    ...
    ftw_in  <= FTW_STEPS(idx_escalon);
    freq_hz <= FREQ_STEPS(idx_escalon);

    -- ---------------------------------------------------------------------
    -- Fuente trifasica de entrada ideal, frecuencia variable
    -- ---------------------------------------------------------------------
    process(clk)
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                phase_acc_U <= phase_acc_U + ftw_in;
                phase_acc_V <= phase_acc_V + ftw_in;
                phase_acc_W <= phase_acc_W + ftw_in;
            end if;
        end if;
    end process;
    ...
    InitTest: process
    begin
        report "tb_SVM_FinVar: salida fija 50 Hz, entrada en escalones 40/50/60 Hz";
        rst <= '1';
        enable_SVM <= '0';
    ...
    -- ---------------------------------------------------------------------
    -- Rampa (NCO) de la referencia de salida deseada, FIJA en 50 Hz
    -- ---------------------------------------------------------------------
    process(clk)
    begin
        if rising_edge(clk) then
    ...
    begin
        ...
        report "tb_SVM_FinVar: fin de los 3 escalones de frecuencia" severity failure;
        wait;
    ...
    gen_csv_tensiones: process
        file file_handler : text open write_mode is "F:\FPGA\Potencia_FPGA\MatrixConverter\SW\matlab\Clk10M_FiVar_50o.csv";
        variable row      : line;
    ...
    begin
        ...
        loop
            ...
            if v_decim = 0 then
                ...
                write(row, v_real_W);
                write(row, string'(","));
                write(row, freq_hz);                -- frecuencia de la fuente de entrada
                writeline(file_handler, row);
    ...
    gen_w_direcciones_csv: process
        file file_w : text open write_mode is "F:\FPGA\Potencia_FPGA\MatrixConverter\SW\matlab\w_direcciones_FiVar.csv";
        variable row : line;
